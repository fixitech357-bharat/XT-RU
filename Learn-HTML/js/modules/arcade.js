...
window.HTMLMaster.modules.Arcade = (function() {
  ...
  function playPanel(game, wasCompleted) {
    return `<section class="arcade-play-panel" id="arcadePlayPanel"><div class="arcade-concept"><span class="eyebrow">Concept</span><h3>${escape(game.title)}</h3><p>This challenge teaches <strong>${escape(game.concept)}</strong>. Read the example, choose the best next execution step, then see the moving explanation.</p><pre>${escape(exampleFor(game))}</pre></div><div class="arcade-challenge"><span class="eyebrow">${escape(game.mode)} Challenge</span><h3>Which step should happen next?</h3><div class="arcade-choices"><button onclick="window.HTMLMaster.modules.Arcade.answer(this, true)">Apply the concept and inspect the next state</button><button onclick="window.HTMLMaster.modules.Arcade.answer(this, false)">Skip the concept and guess randomly</button><button onclick="window.HTMLMaster.modules.Arcade.answer(this, false)">Change unrelated code first</button></div><div id="arcadeFeedback"></div></div></section>`;
  }
  ...
  return { render, play, answer, next, catalog };
})();
